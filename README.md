...
Mostly for speed: they bypass heavy render calculations, which makes real-time rendering fast. That's particularly useful for WebGL and WebGPU renders.
...
The torus needs nothing but its own vertex normals: no lights, no BRDF (no roughness/metalness parameters).
...
Perfect-gui offers the possibility to create image-buttons, which is the reason it can be so useful
